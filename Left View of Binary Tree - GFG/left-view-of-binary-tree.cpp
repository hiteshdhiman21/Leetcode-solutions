...
/*vector<int> leftView(Node *root)
{
    ...
    while(!q.empty()){
        ...
        int levelCur = q.front().second;
        q.pop();
        
        if(ans.size() == levelCur) ans.push_back(cur->data);
        
        if(cur->left) q.push(make_pair(cur->left, levelCur+1));
        if(cur->right) q.push(make_pair(cur->right, levelCur+1));
